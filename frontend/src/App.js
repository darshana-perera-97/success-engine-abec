...
function App({ initialView = "dashboard" }) {
  ...
  useEffect(() => {
    ...
    const loadTeamRequests = async () => {
      const [countryResult, detailResult, waveOffResult, removalResult, intakeResult, refundResult, branchResult, amountChangeResult] = await Promise.all([
        ...
        getInvoiceAmountChangeRequests({ pendingOnly: true }),
      ]);
      if (cancelled) return;
      ...
      const amountChangeCount = amountChangeResult.ok && Array.isArray(amountChangeResult.data) ? amountChangeResult.data.length : 0;
      setTeamRequestsCount(countryCount + detailCount + waveOffCount + removalCount + intakeCount + refundCount + branchCount + amountChangeCount);
    };
    loadTeamRequests();
  ...
  const handleAssignStudentCounselor = async (student, counselorId, counselorName = "", branchWhatsappMessengerUserId = "") => {
    ...
    const whatsappPatch = String(branchWhatsappMessengerUserId || "").trim()
      ? { branchWhatsappMessengerUserId: String(branchWhatsappMessengerUserId).trim() }
      : {};
  ...
  const handleAddFromRequest = async (requestRow, { counselorId, viewAccessCounselorIds = [], priority: rawPriority, branchWhatsappMessengerUserId = "" }) => {
    ...
    const payload = {
      ...
      ...(String(branchWhatsappMessengerUserId || "").trim()
        ? { branchWhatsappMessengerUserId: String(branchWhatsappMessengerUserId).trim() }
        : {}),
      notes,
