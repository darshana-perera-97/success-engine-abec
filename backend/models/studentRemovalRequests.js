...
const { STUDENT_REMOVAL_REQUESTS_FILE } = require("../config");

function normalizeStudentRemovalRequest(entry) {
...
async function readStudentRemovalRequests() {
  try {
    const raw = await fs.readFile(STUDENT_REMOVAL_REQUESTS_FILE, "utf8");
    const parsed = safeJsonParse(raw, STUDENT_REMOVAL_REQUESTS_FILE);
    return Array.isArray(parsed) ? parsed.map(normalizeStudentRemovalRequest) : [];
  } catch (error) {
...
async function writeStudentRemovalRequests(list) {
  const normalized = Array.isArray(list) ? list.map(normalizeStudentRemovalRequest) : [];
  return withFileLock(STUDENT_REMOVAL_REQUESTS_FILE, () =>
    atomicWriteFile(STUDENT_REMOVAL_REQUESTS_FILE, JSON.stringify(normalized, null, 2))
  );
}

async function appendStudentRemovalRequest(entry) {
  const normalized = normalizeStudentRemovalRequest({
    ...entry,
    id: entry?.id || `SRR-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`,
    status: "pending",
  ...
  return withFileLock(STUDENT_REMOVAL_REQUESTS_FILE, async () => {
    const list = await readStudentRemovalRequests();
    const hasPending = list.some(
      (row) => row.studentId === normalized.studentId && row.status === "pending"
    );
    if (hasPending) {
      return { ok: false, error: "A pending student removal request already exists for this student." };
    }
    list.unshift(normalized);
    await atomicWriteFile(STUDENT_REMOVAL_REQUESTS_FILE, JSON.stringify(list, null, 2));
    return { ok: true, data: normalized };
  });
}

async function decideStudentRemovalRequest(requestId, decision, reviewer = {}) {
  ...
  return withFileLock(STUDENT_REMOVAL_REQUESTS_FILE, async () => {
    ...
    const updated = normalizeStudentRemovalRequest({
      ...current,
...
module.exports = {
  normalizeStudentRemovalRequest,
  readStudentRemovalRequests,
  writeStudentRemovalRequests,
